...
def _get_fisheye_map(cam):
    if hasattr(cam, '_fisheye_map'): return cam._fisheye_map
    # Virtual wide pinhole camera to cover the fisheye FOV (~115 deg)
    w_wide, h_wide = 800, 800
    f_wide = 200.0
    K_wide = np.array([[f_wide, 0, w_wide/2], [0, f_wide, h_wide/2], [0, 0, 1]])
...
def render(scene, cam, R_wc, t_wc, background=0):
    ...
    if cam.distortion == 'equidistant':
        image = cv2.remap(image, map_x, map_y, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=background)
        depth = cv2.remap(depth.astype(np.float32), map_x, map_y, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=np.inf)
